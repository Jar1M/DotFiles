...
function handleEnum(obj, cb) {
    var props = [];
    for (var prop in obj) {
        if (handleHasProperty(obj, prop) && prop[0] !== '_') {
            props.push(prop);
        }
    }
...
function handleHasProperty(obj, prop) {
	return obj.hasOwnProperty(prop) || obj === window.location && ['href', 'protocol', 'host', 'hostname', 'port', 'pathname', 'search', 'hash', 'username', 'password', 'origin'].indexOf(prop) >= 0 && typeof obj[prop] === 'string';
}
function handleGetP(wyrmlingStore, obj, prop, cb) {
	if (!handleHasProperty(obj, prop) && !(prop === 'length' && (isArray(obj) || isFunction(obj)))) {
